...
python db_init.py
 ...
 Para que tu entorno de desarrollo quede completamente funcional y soporte estas librerías, aquí tienes la solución definitiva:

Paso 1: Instalar CMake desde Python
CMake es el orquestador que organiza el código antes de compilarlo. Puedes instalarlo rápidamente dentro de tu entorno virtual ejecutando:


pip install cmake


Paso 2: Instalar Visual Studio C++ Build Tools

Descarga las Build Tools for Visual Studio desde la página oficial de Microsoft (no necesitas el IDE completo de Visual Studio, solo las "Herramientas de compilación").

Ejecuta el instalador. Te mostrará un menú con varias opciones.

Marca la casilla que dice "Desarrollo para el escritorio con C++" (Desktop development with C++).

En el panel de la derecha, asegúrate de que estén seleccionadas las opciones por defecto (especialmente el compilador MSVC v143 y el SDK de Windows).

Haz clic en Instalar. (Toma en cuenta que esto descargará un par de gigabytes, por lo que puede tardar un rato).

Paso 3: Reiniciar y reintentar
Una vez que termine la instalación de Microsoft:

Cierra tu consola actual de PowerShell.

Abre una nueva consola, vuelve a navegar a la carpeta backend y activa nuevamente tu entorno virtual: 

.venv\Scripts\activate.

Ejecuta de nuevo tu comando original (Estando dentro de la carpeta Backend):
